...
export class UsersRepository {
  ...
  async createUser(
    ...
    let user;
    await this.dataSource.query(
      `INSERT INTO users ("login", "email", "password", "isConfirmed", "expirationDate") VALUES ('${login}', '${email}', '${password}', '${isConfirmed}', '${date}');`,
    );
    if (isConfirmed) {
      const user = (
        await this.dataSource.query(
          `SELECT "login", "email", "id", "createdAt" From users where "login" = '${login}' and "email" = '${email}'`,
        )
      )[0];
  ...
  async confirmUserByCode(code: string) {
    const user = (
      await this.dataSource.query(
        `SELECT * from users WHERE "confirmationCode" = '${code}'`,
      )
    )[0];
    ...
    await this.dataSource.query(
      `UPDATE users SET "isConfirmed" = true WHERE "confirmationCode" = '${code}'`,
    );
    // await user.confirm();
    // await user.save();
    // console.log(user);
