...
export class UsersService {
    constructor(@InjectModel(User.name) private userModel : Model<User>, private readonly cryptoService : CryptoService, private usersRepository : UsersRepository){}

    async getAllUsers(paganation:IUsersPaganationQuery){

        const searchLoginTerm = (paganation.searchLoginTerm) ? paganation.searchLoginTerm : ''

        const searchEmailTerm = (paganation.searchEmailTerm) ? paganation.searchEmailTerm : ''
    
        const sortBy = (paganation.sortBy) ? paganation.sortBy : "createdAt"
    
        const sortDirection = (paganation.sortDirection === "asc") ? 1 : -1
        
        const sotringQuery = this.usersRepository.usersSortingQuery(sortBy, sortDirection)
        ...
        const totalCountOfItems = (await this.userModel.find({ $or: [{login: {$regex: searchLoginTerm, $options: 'i'}},{email: {$regex: searchEmailTerm, $options: 'i'}}]})).length
