...
export class UsersRepository {
  ...
  async getAllUsers(paganation: IUsersPaganationQuery) {
    ...
    const usersToSend = await this.usersRepo
      .createQueryBuilder('users')
      .select(['users.id', 'users.login', 'users.email', 'users.createdAt'])
      .where('LOWER(users.login) LIKE LOWER(:searchLoginTerm)', {
        searchLoginTerm: `%${searchLoginTerm}%`,
      })
      .orWhere('LOWER(users.email) LIKE LOWER(:searchEmailTerm)', {
    ...
    const totalCountOfItems = await this.usersRepo
      .createQueryBuilder('users')
      .where(`LOWER (users.login) LIKE LOWER (:searchLoginTerm)`, {
        searchLoginTerm: `%${searchLoginTerm}%`,
      })
  ...
  async newConfirmationCode(id: string) {
    ...
    await this.usersRepo.update(
      { id: parseInt(id) },
      { confirmationCode: newCode, expirationDate: +new Date() + 180000 },
    );
    return newCode;
  }
}
