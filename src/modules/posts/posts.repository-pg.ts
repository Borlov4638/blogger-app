...
export class PostRepositoryPg {
  ...
  async getAllPosts(
    ...
    const sortBy = postPagonationQuery.sortBy
      ...
      : 'createdAt';
    const sortDirection =
      postPagonationQuery.sortDirection === 'asc' ? 'asc' : 'desc';
    ...
    const postToReturn = findedPosts.map((p) => {
      const post = {
        ...
        extendedLikesInfo: {
          ...
          myStatus: p.status,
          newestLikes: p.likes_array.slice(0, 3).map((u) => {
            return { ...u, userId: u.userId.toString() };
          }),
        },
      };
      return post;
    ...
    const totalCountOfItems = (
      await this.dataSource.query(`SELECT * FROM posts`)
    ).length;
  ...
  async createPost(blogToPost, data) {
    const createdPost: IPostPostgres = (
      await this.dataSource
        .query(`INSERT INTO posts ("title", "shortDescription", "content", "blogId") 
  ...
  async findPostById(postId: string) {
    ...
    const postsLikes: IPostsLikes[] = await this.dataSource.query(
      `SELECT * FROM posts_likes WHERE "postId" = '${postId}'`,
    );
