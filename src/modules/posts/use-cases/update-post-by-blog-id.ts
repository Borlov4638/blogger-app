...
import { GetAllPostsInBlogCommand } from './get-posts-by-blog-id';
...
export class UpdatePostAssignedToBlogCommand {
  constructor(
    ...
    public data: IPostUpdate,
  ) {}
}
@CommandHandler(UpdatePostAssignedToBlogCommand)
export class UpdatePostAssignedToBlogUseCase
  implements ICommandHandler<UpdatePostAssignedToBlogCommand>
{
  ...
  async execute(command: UpdatePostAssignedToBlogCommand): Promise<any> {
    ...
    await this.commandBus.execute(
      new UpdatePostCommand(command.postId, command.data),
    );
    return;
  }
}
