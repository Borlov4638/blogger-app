...
export class ValidateSessionUseCase implements ICommandHandler<ValidateSessionCommand>{
    constructor(private sessionRepo: SessionRepository) { }

    async execute(command: ValidateSessionCommand) {
        const refreshHash = command.request.cookies.refreshToken.split('.')[2]
        const session = await this.sessionRepo.findSessionByRefreshHash(refreshHash);
        if (!session) {
