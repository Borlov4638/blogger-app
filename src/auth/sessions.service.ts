...
export class SessionService{
    constructor(@InjectModel(Session.name) private sessionModel: Model<Session>, private jwtService : JwtService){}
    async createNewSession(req: Request, user : UserDocument ,expDate: number){
        ...
        const newSession = new this.sessionModel({userId:user.id, deviceId, ip:requestIp, title:userAgent, lastActiveDate:new Date(lastActiveDate).toISOString(), expiration:refreshTokenExpirationDate})
    ...
    async updateCurrentSession(req: Request, expDate: number, deviceId:string){
        ...
        const updatedSession = await this.sessionModel.findOneAndUpdate({deviceId},{ip:requestIp, title:userAgent, lastActiveDate: new Date(lastActiveDate).toISOString(), expiration:refreshTokenExpirationDate})
