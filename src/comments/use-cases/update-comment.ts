...
export class UpdateCommentByIdCommand {
    constructor(public newContent: string, public commentId: string, public request: Request) { }
}

@CommandHandler(UpdateCommentByIdCommand)
export class UpdateCommentByIdUseCase implements ICommandHandler<UpdateCommentByIdCommand>{
    constructor(
    ...
    async execute(command: UpdateCommentByIdCommand) {
        ...
        const comment = await this.commentRepo.updateComment(command.commentId, command.newContent)
        if (!comment) {
