...
import { DeleteCommentByIdCommand } from './use-cases/delete-comment';
...
export class CommentsController {
  ...
  async getCommentById(@Param('id') comentId: string, @Req() request: Request) {
    return await this.commandBus.execute(
      new GetCommentByIdCommand(comentId, request),
    );
  }
  ...
  async updateComment(
    ...
    return await this.commandBus.execute(
      new UpdateCommentByIdCommand(data.content, commentId, request),
    );
  }
  ...
  async changessLikeStatus(
    ...
    return await this.commandBus.execute(
      new ChengeCommentLikeStatusCommand(request, id, body.likeStatus),
    );
  }
  ...
  async deleteCommentById(
    ...
    await this.commandBus.execute(
      new DeleteCommentByIdCommand(commentId, request),
    );
    return;
  }
}
