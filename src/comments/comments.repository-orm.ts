...
export class CommentRepositoryPg {
  ...
  async createComment(commentatorInfo, content: string, postId: string) {
    const user = await this.dataSource.createQueryBuilder().select("u").from(UsersEntity, "u").where({ id: Number(commentatorInfo.userId) }).getOne()
    const post = await this.dataSource.createQueryBuilder().select("p").from(PostEntity, "p").where({ id: Number(postId) }).getOne()
    console.log(user)
