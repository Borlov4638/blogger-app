...
export class PostsService {
  ...
  async getAllPosts(postPagonationQuery: IPostPaganationQuery, request:Request) {
    ...
    const postsToShow = findedPosts.map(post =>{
      ...
      //@ts-ignore
      const newestLikes  = post.likesInfo.usersWhoLiked.sort((a,b) => b.addedAt - a.addedAt).slice(0,3)
  ...
  async getPostById(postId: string, request:Request) {
    ...
    //@ts-ignore
    const newestLikes  = findedPost.likesInfo.usersWhoLiked.sort((a,b) => b.addedAt - a.addedAt).slice(0,3)
  ...
  async getAllPostsInBlog(
    ...
    const findedPosts = await this.postModel
      ...
      const postsToShow = findedPosts.map(post =>{
        ...
        try{
        //@ts-ignore
          newestLikes  = post.likesInfo.usersWhoLiked.sort((a,b) => b.addedAt - a.addedAt).slice(0,3)
        }
        catch{
          newestLikes = []
        }
